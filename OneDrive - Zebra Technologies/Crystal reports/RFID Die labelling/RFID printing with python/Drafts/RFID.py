...
def load_grf_to_zpl(filename):
    ...
    bytes_per_row = 56  # Adjust according to your image's byte width, if necessary
...
def generate_zpl(label_data):
    ...
    # Load a single image based on the weight
    weight = label_data['weight']
    
    if weight > 30:
        zpl_commands.append(load_grf_from_file("1Weight.grf"))  # Load image for Very Heavy
    elif weight > 25:
        zpl_commands.append(load_grf_from_file("2Weight.grf"))  # Load image for Heavy
    else:
        zpl_commands.append(load_grf_from_file("3Weight.grf"))  # Load image for Manual Handling
    ...
    zpl_commands.append(f"^FO311.853,64.816^A0N,102,102^FD{label_data['teeth']} T^FS")  # Teeth
    ...
    # Other label data
    zpl_commands.append(f"^FO175.290,255.621^A0N,39,39^FD{label_data['spec_code']}^FS")  # SpecCode
    zpl_commands.append(f"^FO175.811,315.209^BY2,2.0,80^BCN,N,N,Y,N,N^FD{label_data['tool_code']}^FS")  # ToolCode Barcode
    zpl_commands.append(f"^FO599.848,255.622^A0N,39,39^FDLegacy Code:^FS")  # Legacy Code Text
    zpl_commands.append(f"^FO599.526,335.728^A0N,39,39^FD{label_data['legacy_code']}^FS")  # Legacy Code
...
def load_grf_from_file(filename):
    """Load GRF data from the file and return it as is."""
    with open(filename, 'r') as file:
        return file.read().strip()  # Return the content as is
